...
OWN="rtl/xb_pkg.sv rtl/video/xb_video_timing.sv rtl/mem/sdram.sv rtl/mem/xb_rom_loader.sv rtl/mem/xb_dpram.sv \
  ...
  rtl/cpu/xb_rom_cache.sv rtl/cpu/xb_m68k_bus.sv rtl/xb_core.sv"
...
for f in $OWN; do
  ...
  verilator --lint-only $W rtl/xb_pkg.sv $f --top-module $(basename $f .sv) >/dev/null
done
# board with fx68k inside: fx68k's own warnings waived, ours still fatal
verilator --lint-only $W $FX $OWN $FXF --top-module xb_core 2>&1 | grep -E "%Warning|%Error" | grep -v "Exiting due" && exit 1
echo "lint clean"
