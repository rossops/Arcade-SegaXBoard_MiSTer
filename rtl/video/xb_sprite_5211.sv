...
typedef enum logic [3:0] {
    ...
    R_ROMREQ, R_ROMWAIT, R_PIX, R_ROWEND, R_NEXT
} rs_t;
rs_t rs;
...
reg  [9:0] x;
reg [11:0] xacc;            // xacc < 0x200 + hzoom
...
wire       on_screen_y = (y >= 0) && (y < 224);   // MAME clips to the screen
// framebuffer x window: MAME clips to screen x 0..319 = fb x 190..509
wire signed [10:0] sx = $signed({1'b0, x}) - 11'sd190;
wire       on_screen_x = (sx >= 0) && (sx < 320);
...
always @(posedge clk) begin
    if (reset) fb_rd_req <= 1'b0;
        ...
        if (fb_rd_req) begin if (fb_rd_ack) fb_rd_req <= 1'b0; end
            ...
            fb_rd_y   <= (vcnt == 9'd261) ? 8'd0 : vcnt[7:0] + 8'd1;
        end
    end
end
...
always @(posedge clk) begin
    ...
    if (reset) begin
        ...
        R_ERASEW: begin
            if (fb_er_ack) begin
                fb_er_req <= 1'b0;
                if (er_line == 8'd255) rs <= R_FETCH;
                else begin er_line <= er_line + 8'd1; rs <= R_ERASE; end
            end
        end
        ...
        R_DECODE: begin
            if (w[0][15]) begin rs <= R_IDLE; rendering <= 1'b0; end   // end of list
                ...
                top    <= $signed({1'b0, w[0][8:0]}) - 10'sd256;
                addr   <= w[1];
                pitch  <= $signed({w[4][12], w[2][15:9]});
                xpos   <= (w[2][8:0] < 9'h80 && !w[4][13]) ? ({1'b0, w[2][8:0]} + 10'h200) : {1'b0, w[2][8:0]};
                vzoom  <= (w[3][10:0] < 11'h40) ? 11'h40 : w[3][10:0];
                hzoom  <= (w[4][10:0] < 11'h40) ? 11'h40 : w[4][10:0];
                ydown  <= w[4][15];
                flip   <= ~w[4][14];
                xright <= w[4][13];
                height <= w[5][11:0] + 12'd1;
        ...
        R_ROW: begin
            if (hide) rs <= R_NEXT;
                ...
                y <= top; rows_left <= height; yacc <= 9'd0;
                rs <= R_ROWWAIT;
            end
        end
        // start a row: wait for the previous run's flush, then open a run
        R_ROWWAIT: begin
            if (rows_left == 12'd0) rs <= R_NEXT;
            else if (!on_screen_y) begin
                // skip the row but advance the base address exactly like MAME
                {yacc, addr} <= next_row(yacc, addr);
                y <= ydown ? y + 10'sd1 : y - 10'sd1;
                rows_left <= rows_left - 12'd1;
            end
        ...
        R_ROMREQ: begin
            if (!on_screen_x && ((xright && sx >= 320) || (!xright && sx < 0))) begin
        ...
        // emit one framebuffer pixel per clock while xacc < 0x200
        R_PIX: begin
            if (xacc < 12'h200) begin
                fb_wr_valid <= on_screen_x && (pix != 4'd0) && (pix != 4'hF);
                fb_wr_pix   <= colpri | {12'd0, pix};
                fb_wr_x     <= x[8:0];
                x    <= xright ? x + 10'd1 : x - 10'd1;
                xacc <= xacc + {1'b0, hzoom};
            end
            else begin
                xacc <= xacc - 12'h200;
                pixels <= pixels >> 4;
...
function automatic [24:0] next_row(input [8:0] ya, input [15:0] ra);
    logic [11:0] sum;
    logic signed [15:0] step;
    sum  = {3'd0, ya} + {1'b0, vzoom};
    step = $signed(pitch) * $signed({1'b0, sum[11:9]});
    next_row = {sum[8:0], ra + step[15:0]};
endfunction
endmodule
